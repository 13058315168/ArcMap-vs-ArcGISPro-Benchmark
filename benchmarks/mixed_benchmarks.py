...
class M1_PolygonToRaster(BaseBenchmark):
    ...
    def setup(self):
        ...
        if self.output_format == 'GDB':
            self.output_raster = os.path.join(gdb_path, "M1_poly_to_ras")
        else:
...
class M2_RasterToPoint(BaseBenchmark):
    ...
    def setup(self):
        ...
        gdb_path = get_benchmark_gdb_path(settings.DATA_DIR)
        if self.output_format == 'GDB':
            self.output_fc = os.path.join(gdb_path, "M2_ras_to_point")
        else:
    ...
    def run_single(self):
        ...
        return _run_repeated_operation(
            self.repeat_count,
            lambda: remove_dataset_artifacts(self.output_fc),
            _operation
        )
